...
![A 13 by 13 maze in thin dark lines on off-white paper. A grey trail fills the bottom left of the board, running back and forth along the lowest rows and doubling back on itself several times; it is darkest where the runner is and fades towards its oldest end. The amber runner is a filled dot at that dark end, against the left side of the board a little below the middle. A dashed amber line runs up from the runner, turns right, drops back down, steps right again and ends at an amber arrow near the middle of the board that points left. A small ringed cell sits against the right-hand wall about four fifths of the way down. The HUD above the board reads 1 cleared and 22.7 s left, the button below it says Turn, and the hint under the button reads: Space, Enter, or tap: move the arrow to the next exit.](screenshot.png)
